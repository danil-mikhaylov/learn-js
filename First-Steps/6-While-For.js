...
while(True) {
    ...
    if (!value) { // Если value = false, следовательно, !value = true, следовательно, выполнится break;
        break;
    }
    sum += value; 
}
...
//continue 
for (let i = 0;i < 10; i++) { // Выводит все нечётные числа до 10
...
for (let i = 0; i < 10; i++) { // Тот же самый вариант
    if (i % 2) { // Если число нечётное, то...
      alert( i ); // ...вывод
    }
}
...
for (let i = 0; i < 10; i++) { // Цикл до 10, не включая 10
    if (i % 2 == 0) { // Если остаток = 0, то есть число чётное, то
