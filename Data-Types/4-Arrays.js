...
function getMaxSubSum(arr) { // Аргмуент функции -- массив, то есть он подаётся на вход
  let maxSum = 0; // Создаём перменную максимальной суммы, которую в конце и будем выводить
  let partialSum = 0; // Сумма "сейчас"
  for (let item of arr) { // Для каждого элемента в массиве (item -- замена "i")
    partialSum += item; // К сумме "сейчас" прибавляем текущий элемент
    maxSum = Math.max(partialSum, maxSum); /* Выбираем максимальное из суммы сейчас (которая...
                                          может быть < 0, и maxSum( которая равна 0)
*/
    if (partialSum < 0) { // Если всё же сумма "сейчас" отрицательна, мы её делаем 0
      partialSum = 0; 
    }
  }
